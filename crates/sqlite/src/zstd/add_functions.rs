use super::dict_training::ZstdTrainDictAggregate;
use super::util::*;
use super::{basic::zstd_decompress_fn, transparent::*};

use super::basic::zstd_compress_fn;
...
pub mod tests {
    ...
    pub fn create_example_db(seed: Option<u64>, eles: i32) -> anyhow::Result<Connection> {
        ...
        let mut db = if std::env::var("TEST_TO_FILE").is_ok() {
            let db_fname = format!("/tmp/foo.{}.sqlite3", rand::random_range(0..10000));
        ...
        let app_id_dist = Uniform::new(0, 100)?;
        let property_count_dist = Uniform::new(100, 1000)?;
        let data = (0..eles).map(|_| match event_type_dist.sample(&mut rng) {
            0 => {
                let mut properties = BTreeMap::new();
                for _i in 1..property_count_dist.sample(&mut rng) {
