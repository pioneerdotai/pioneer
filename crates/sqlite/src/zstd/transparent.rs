...
pub fn zstd_enable_transparent<'a>(ctx: &Context) -> anyhow::Result<ToSqlOutput<'a>> {
    ...
    }
    {
        ...
        db.execute(
            &format_sqlite!(
                "create index {} on {} ({})",
                &format!("{}__{}_idx", &new_table_name, &dict_id_column_name),
                &new_table_name,
                &dict_id_column_name
            ),
            params![],
...
mod tests {
    ...
    fn row_to_thong(r: &Row) -> anyhow::Result<Vec<Value>> {
        Ok((0..r.as_ref().column_count())
            .map(|i| r.get_ref(i).map(value_ref_to_owned))
            .collect::<Result<_, _>>()?)
    }
    ...
    #[test]
    fn same_column_name_can_be_enabled_on_different_tables() -> anyhow::Result<()> {
        let db = create_example_db(Some(123), 100)?;
        db.execute(
            "create table other_events (id integer primary key autoincrement, data text not null)",
            params![],
        )?;
        for index in 0..100 {
            db.execute(
                "insert into other_events (data) values (?)",
                params![format!(
                    r#"{{"other": {index}, "payload": "{}"}}"#,
                    "x".repeat(128)
                )],
            )?;
        }
        ...
        db.query_row(
            ...
            params![
                r#"{"table": "events", "column": "data", "compression_level": 3, "dict_chooser": "'events.data'"}"#
            ],
            |_| Ok(()),
        )
        .context("enable transparent for events.data")?;
        db.query_row(
            r#"select zstd_enable_transparent(?)"#,
            params![
                r#"{"table": "other_events", "column": "data", "compression_level": 3, "dict_chooser": "'other_events.data'"}"#
            ],
            |_| Ok(()),
        )
        .context("enable transparent for other_events.data")?;

        let event_indexes: i64 = db.query_row(
            "select count(*) from pragma_index_list('_events_zstd') where name like '%data_dict%'",
            params![],
            |row| row.get(0),
        )?;
        let other_event_indexes: i64 = db.query_row(
            "select count(*) from pragma_index_list('_other_events_zstd') where name like '%data_dict%'",
            params![],
            |row| row.get(0),
        )?;
        assert_eq!(event_indexes, 1);
        assert_eq!(other_event_indexes, 1);
