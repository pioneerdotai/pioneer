...
pub(crate) fn validate_protocol_version(headers: &HeaderMap) -> Result<(), InvalidProtocolVersion> {
    let values = headers
        .get_all(PIONEER_PROTOCOL_VERSION_HEADER)
        .iter()
        .collect::<Vec<_>>();
    if values.len() != 1 || values[0].as_bytes() != PIONEER_PROTOCOL_VERSION.as_bytes() {
