...
async fn compaction_is_current(crud_store: &CrudStore) -> Result<bool> {
    ...
    if meta.projection_version != AGENT_DIFF_EVENT_COMPACTION_VERSION
        || meta.status != PROJECTION_META_STATUS_COMPLETE
    {
        return Ok(false);
    }

    let remaining = crud_store
        .compact_superseded_agent_diff_turn_events(1, true)
        .await
        .context("failed to check current agent diff compaction state")?;
    Ok(remaining.candidate_rows == 0)
}
