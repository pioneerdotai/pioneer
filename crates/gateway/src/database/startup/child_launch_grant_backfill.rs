        ...
        .run_background_database_quantum(|| {
            ...
            async move {
                ...
                let result = match target {
                    BackfillTarget::TaskActorContracts => {
                        backfill_task_actor_contract_batch(&transaction, after_id.as_deref()).await
                    }
                    BackfillTarget::ExecutionGrants => {
                        backfill_execution_grant_batch(&transaction, after_id.as_deref()).await
                    }
                };
                match result {
                    Ok(summary) => {
...
async fn backfill_task_actor_contract_batch(
    ...
    for row in rows {
        ...
        active.derived_child_launch_grant_json = Set(migrated.derived_child_launch_grant_json);
        active
            .update(database)
            .await
...
async fn backfill_execution_grant_batch(
    database: &DatabaseTransaction,
    after_id: Option<&str>,
) -> Result<BackfillBatchResult> {
    ...
    for row in rows {
        ...
        let mut active = row.into_active_model();
        active.grant_json = Set(migrated.grant_json);
        active.grant_fingerprint = Set(migrated.grant_fingerprint);
        active
            .update(database)
            .await
