...
use crate::screen::{SettingsContentView, SettingsSidebarView};
...
impl SettingsSidebarView {
    pub(crate) fn render_settings_sidebar(&self, cx: &mut Context<Self>) -> AnyElement {
        ...
        let tree_view = tree(
            &settings_tree_state,
            move |_ix, entry, selected, window, cx| {
                ...
                match parse_settings_sidebar_node_key(item_id) {
                    SettingsSidebarNodeKey::Content(content_view) => {
                        ...
                        ListItem::new((
                            ElementId::Name("settings-tree-row".into()),
                            entry.item().id.clone(),
                        ))
                        .separator()
                        ...
                        .child(
                            div()
                                .id((
                                    ElementId::Name("settings-tree-row-click".into()),
                                    entry.item().id.clone(),
                                ))
                                .w_full()
                                .h(px(TREE_ROW_HEIGHT_PX))
                    ...
                    SettingsSidebarNodeKey::Unknown => ListItem::new((
                        ElementId::Name("settings-tree-row".into()),
                        entry.item().id.clone(),
                    ))
                    .separator()
