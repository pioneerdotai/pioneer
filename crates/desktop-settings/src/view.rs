...
impl SettingsScreenView {
    ...
    fn render_settings_general(&self, _window: &mut Window, cx: &mut Context<Self>) -> AnyElement {
        let selected_language = self.config.platform.language(cx);
        let selected_theme = self.config.platform.theme(cx);
    ...
    fn render_file_opener_setting(
        ...
        let available = self.config.platform.available_file_openers();
        h_flex()
            .debug_selector(|| "settings-file-opener-form".to_string())
            .w_full()
            .gap_6()
            .py_3()
            .justify_between()
    ...
    pub(crate) fn render_voice_input_setting(
        &self,
    ...
    fn render_voice_input_download_progress(
        ...
        h_flex()
            .id(progress_id)
            .debug_selector(move || progress_id.to_string())
            .gap_2()
            .mb_0p5()
            .items_center()
    ...
    pub(crate) fn render_remote_access_setting(
        &self,
        ...
        let key_input = self.remote_key.clone();
        let status_label = Self::remote_access_status_label(&settings);

        v_flex()
            .debug_selector(|| "settings-remote-access-form".to_string())
            .w_full()
            .gap_0()
            .child(
                h_flex()
    ...
    fn theme_icon(preference: WindowThemePreference) -> AnyElement {
        match preference {
            WindowThemePreference::System => Icon::empty()
                .path("icons/sun-moon.svg")
                .size_3p5()
                .into_any_element(),
...
#[cfg(test)]
mod tests {
    use super::{GatewayVoiceInputRuntimePhase, SettingsScreenView};
    use pioneer_client::settings::types::{
    ...
    fn remote_access_status_label_follows_switch_state() {
        ...
        assert_eq!(
            super::SettingsScreenView::remote_access_status_label(&settings),
    ...
    fn voice_settings_enable_flow_is_gateway_authoritative_and_selector_first() {
        ...
        let general_view = source
            ...
            .next()
            .expect("general renderer body exists");
        assert!(general_view.contains("self.voice.clone()"));
    ...
    fn voice_settings_runtime_states_have_deterministic_labels_and_progress() {
        ...
        for (phase, expected) in phases {
            assert_eq!(
                SettingsScreenView::voice_input_runtime_phase_label(phase),
                expected
            );
        }

        let known = SettingsScreenView::voice_input_download_presentation(
            Some(50 * 1024 * 1024),
            Some(100 * 1024 * 1024),
        );
        ...
        let clamped = SettingsScreenView::voice_input_download_presentation(Some(120), Some(100));
        assert_eq!(clamped.fraction, Some(1.0));
        assert!(clamped.label.starts_with("100%"));

        let unknown = SettingsScreenView::voice_input_download_presentation(Some(2048), None);
        assert_eq!(unknown.label, "0.0 MB downloaded - size unknown");
        assert_eq!(unknown.fraction, None);

        assert_eq!(
            SettingsScreenView::format_voice_input_megabytes(1024 * 1024 * 1024),
            "1024.0 MB"
        );
    }
    ...
    fn vector_search_saved_model_selection_is_reused_on_enable() {
        ...
        assert_eq!(
            SettingsScreenView::vector_search_model_selection(&local),
            (
                Some("local".to_owned()),
        ...
        assert_eq!(
            SettingsScreenView::vector_search_model_selection(&remote),
            (
    ...
    fn vector_search_missing_or_empty_model_still_requires_selection() {
        ...
        assert_eq!(
            SettingsScreenView::vector_search_model_selection(&missing_provider),
            (None, None)
        );
        ...
        assert_eq!(
            SettingsScreenView::vector_search_model_selection(&empty_model),
    ...
    fn voice_settings_match_vector_search_control_structure() {
        ...
        assert!(voice_view.contains("progress.clone()"));
