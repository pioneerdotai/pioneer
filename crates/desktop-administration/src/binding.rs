...
pub(crate) struct AdministrationBinding {
...
impl ClientPublicationSink for AdministrationBinding {
    fn publish(&self, publication: ClientPublicationReference) {
        ...
        self.changed.send_modify(|revision| {
            *revision = revision
                .checked_add(1)
                .expect("administration binding revision exhausted")
        });
    }
}
