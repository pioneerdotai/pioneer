//! `SeaORM` Entity, @generated by sea-orm-codegen 2.0

pub mod prelude;

pub mod actor_nickname_index;
pub mod agent_action;
pub mod agent_action_outbox;
pub mod agent_action_receipt;
pub mod agent_action_timeline_target;
pub mod agent_delegation_route;
pub mod agent_delegation_route_event;
pub mod agent_domain_commit;
pub mod agent_execution;
pub mod agent_execution_grant;
pub mod agent_execution_resource_state;
pub mod agent_identity;
pub mod agent_memory;
pub mod agent_memory_candidate;
pub mod agent_memory_capsule;
pub mod agent_memory_event;
pub mod agent_memory_identity;
pub mod agent_memory_policy_decision;
pub mod agent_memory_quality_decision;
pub mod agent_memory_quarantine;
pub mod agent_memory_repair_job;
pub mod agent_presentation_snapshot;
pub mod agent_running_permit;
pub mod agent_skill;
pub mod agent_skill_version;
pub mod agent_turn_response_execution;
pub mod agent_work_branch_schedule;
pub mod agent_work_queue;
pub mod agent_work_resource_scope;
pub mod agent_work_scheduler_state;
pub mod applied_patch_change_index;
pub mod applied_patch_record;
pub mod artifact;
pub mod artifact_binding;
pub mod artifact_blob;
pub mod artifact_external_ref;
pub mod artifact_projection;
pub mod artifact_upload_session;
pub mod artifact_version;
pub mod audit_event;
pub mod auth_refresh_credential;
pub mod auth_session;
pub mod authorization_change_feed;
pub mod authorization_policy_state;
pub mod cli_runtime_native_event;
pub mod cli_runtime_pending_request;
pub mod codex_aggregate_state;
pub mod compaction_attempt_observation;
pub mod compaction_checkpoint;
pub mod compaction_context;
pub mod compaction_coverage;
pub mod compaction_delivery_output;
pub mod compaction_event_revision;
pub mod compaction_execution_stop;
pub mod compaction_frozen_history;
pub mod compaction_frozen_import;
pub mod compaction_frozen_message;
pub mod compaction_history_check;
pub mod compaction_history_preparation;
pub mod compaction_input_revision;
pub mod compaction_item_revision;
pub mod compaction_manifest;
pub mod compaction_operation;
pub mod compaction_operation_projection;
pub mod compaction_projection_epoch;
pub mod compaction_runner_plan;
pub mod compaction_runner_state;
pub mod compaction_source_revision;
pub mod compaction_task_basis_revision;
pub mod compaction_task_output;
pub mod compaction_turn_creation;
pub mod device;
pub mod execution_admission_lease;
pub mod gateway_identity;
pub mod gateway_principal;
pub mod hook_audit_event;
pub mod hook_run;
pub mod hook_run_attempt;
pub mod invitation;
pub mod invitation_workspace_grant;
pub mod mcp_audit_event;
pub mod mcp_server_catalog_snapshot;
pub mod mcp_server_installation;
pub mod native_agent_config;
pub mod native_terminal_effect_outbox;
pub mod patch_commit_intent;
pub mod patch_commit_terminal;
pub mod patch_snapshot;
pub mod patch_snapshot_reservation;
pub mod principal_avatar;
pub mod principal_avatar_revision;
pub mod read_model_repair_change_sequence;
pub mod read_model_repair_checkpoint;
pub mod read_model_repair_dirty_turn_item;
pub mod recovery_job;
pub mod recovery_terminalization_outbox;
pub mod self_improvement_run;
pub mod self_improvement_source_turn;
pub mod self_improvement_workspace_state;
pub mod skill_audit_event;
pub mod skill_dependency_snapshot;
pub mod skill_installation;
pub mod skill_pack_installation;
pub mod skill_upload_session;
pub mod skill_workspace_policy;
pub mod task;
pub mod task_actor_contract;
pub mod task_agent_spec;
pub mod task_delivery;
pub mod task_delivery_attempt;
pub mod task_delivery_authority;
pub mod task_dependency;
pub mod task_event;
pub mod task_event_fanout_cursor;
pub mod task_execution_admission;
pub mod task_occurrence_contract;
pub mod task_result_candidate;
pub mod task_result_review_event;
pub mod task_run;
pub mod task_run_conversation_snapshot;
pub mod task_run_execution;
pub mod task_run_thread_binding;
pub mod task_run_turn;
pub mod task_trigger;
pub mod task_write_lock;
pub mod thread;
pub mod thread_agents_doc;
pub mod thread_agents_doc_revision;
pub mod thread_cli_runtime_binding;
pub mod thread_episodic_capsules;
pub mod thread_episodic_embedding_artifacts;
pub mod thread_episodic_exclusions;
pub mod thread_episodic_index_jobs;
pub mod thread_episodic_items;
pub mod thread_episodic_recall_events;
pub mod thread_episodic_thread_directory;
pub mod thread_folder;
pub mod thread_lineage;
pub mod thread_membership;
pub mod thread_placement;
pub mod thread_read_cursor;
pub mod thread_sandox_policy;
pub mod thread_timeline_block;
pub mod thread_timeline_projection_meta;
pub mod turn;
pub mod turn_admission;
pub mod turn_cli_runtime_attempt;
pub mod turn_cli_runtime_binding;
pub mod turn_cli_runtime_execution_segment;
pub mod turn_cli_runtime_instruction;
pub mod turn_diff_state;
pub mod turn_event;
pub mod turn_event_delivery;
pub mod turn_event_projection_state;
pub mod turn_event_projection_stream_state;
pub mod turn_execution;
pub mod turn_execution_checkpoint;
pub mod turn_execution_window;
pub mod turn_finalization;
pub mod turn_input;
pub mod turn_item;
pub mod turn_item_attempt;
pub mod turn_liveness;
pub mod turn_llm_context;
pub mod turn_mcp_binding;
pub mod turn_mcp_projection;
pub mod turn_message_revision;
pub mod turn_runtime_snapshot;
pub mod turn_skill_binding;
pub mod turn_status_history;
pub mod turn_work_item_projection;
pub mod turn_work_projection;
pub mod user_notification_outbox;
pub mod workspace;
pub mod workspace_membership;

pub mod tool_output_chunk;
