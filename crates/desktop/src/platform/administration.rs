use gpui_kit::{App, AppContext, ClipboardItem, Task};
use pioneer_client::{
    avatars::{AvatarCacheError, AvatarCacheRequest, AvatarCacheResult},
    core::ClientCore,
};
use pioneer_desktop_administration::{
    AdministrationActivationPort, AdministrationAvatarPort, AdministrationConfig,
    AdministrationEffectCompletion, AdministrationEffectIdentity,
    AdministrationExternalNavigationPort,
};
use std::sync::Arc;
use tokio_util::sync::CancellationToken;

struct DesktopAdministrationPlatform {
...
pub(crate) fn administration_config(client: Arc<ClientCore>, cx: &App) -> AdministrationConfig {
    let platform = Arc::new(DesktopAdministrationPlatform {
        client: client.clone(),
    });
