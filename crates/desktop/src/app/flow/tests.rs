...
use crate::gateway::GatewayInstallWarning;
use crate::gateway::onboarding_platform::{
...
fn local_ws_connect_spec_keeps_configured_timeout() {
    let config = crate::gateway::tests::test_config();
    let timings =
        crate::gateway::timings::gateway_ws_timings_from_config(&config.desktop.gateway).unwrap();
    assert_eq!(timings.connect_timeout, Duration::from_millis(300));
}
