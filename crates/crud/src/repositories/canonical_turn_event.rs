...
use super::self_improvement_source_turn::{
    VerifiedCollaborativeExchange, collaborative_child_lineage_matches, eligible_source_thread,
...
pub async fn list_for_frozen_range<C: ConnectionTrait>(
    ...
    for (thread_id, selected_thread_sources) in sources_by_thread {
        ...
        if !eligible_source_thread(&visible_thread) {
            bail!("canonical history thread `{thread_id}` is no longer eligible");
        }
        let origin =
