...
#[async_trait]
impl crate::traits::Provider for GeminiCliProvider {
    fn name(&self) -> &str {
        "gemini_cli"
    }
    ...
    async fn chat(&self, request: ChatRequest) -> Result<ChatResponse> {
        ...
        if !output.status.success() {
            ...
            return Err(anyhow!("gemini command failed: {stderr}"));
        }
...
mod tests {
    ...
    fn creates_with_custom_path() {
        let provider = GeminiCliProvider::with_path("/usr/local/bin/gemini");
