...
impl TelnyxProvider {
                ...
                let content = if attachments.is_empty() {
                    ...
                    for attachment in attachments {
                        match attachment.kind {
                            ...
                            _ => {
                                return Err(anyhow!(
                                    "provider `telnyx` does not support {:?} attachments in chat/completions yet",
                                    attachment.kind
                                ));
                            }
                        }
    ...
    async fn api_error(response: reqwest::Response) -> anyhow::Error {
        ...
        anyhow!("Telnyx API error ({status}): {body}")
    }
}
...
mod tests {
    ...
    fn creates_with_api_key() {
        let provider = TelnyxProvider::new("KEY_test_123");
        assert_eq!(provider.api_key, "KEY_test_123");
    }

    #[test]
    fn chat_completions_url_built_correctly() {
        let provider = TelnyxProvider::new("key");
        assert_eq!(
    ...
    fn convert_messages_maps_roles() {
        ...
        .unwrap();
        let api_messages = TelnyxProvider::convert_messages(&prepared).unwrap();
