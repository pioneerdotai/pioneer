use crate::editor::AgentsDocumentEditor;
use gpui_kit::component::{
    button::*, input::Editor, scroll::ScrollableElement, theme::ActiveTheme, *,
};
...
impl AgentsDocumentEditor {
    ...
    fn render_conflict_panel(&self, cx: &mut Context<Self>) -> AnyElement {
        let (local_content, remote_doc) = match self.publication.save() {
            AgentsDocEditorSaveState::Conflict {
                local_content,
                remote_doc,
...
impl Render for AgentsDocumentEditor {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        v_flex()
            ...
            .child(match self.publication.load() {
...
fn agents_doc_conflict_preview_pane(
    label: String,
    content: String,
    cx: &mut Context<AgentsDocumentEditor>,
) -> AnyElement {
    v_flex()
        .flex_1()
        .min_w(px(0.))
