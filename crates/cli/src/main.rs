...
fn print_version(json_output: bool) -> Result<()> {
    if json_output {
        ...
        println!("pioneer {}", env!("CARGO_PKG_VERSION"));
    }
...
fn help_text() -> &'static str {
    "Usage:
  pioneer install [--source local|release|channel] [--asset <path> --checksums <path>] [--channel stable|beta|canary] [--version x.y.z]
                  [--managed-by desktop|script|manual|unknown] [--no-start] [--force-start] [--json]
  pioneer update [--source local|release|channel] [--asset <path> --checksums <path>] [--channel stable|beta|canary] [--version x.y.z]
                 [--managed-by desktop|script|manual|unknown] [--no-start] [--force-start] [--json]
  pioneer self-update (alias of `update`)
  pioneer start [--json]      Install and start the persistent gateway service
  pioneer status [--json]     Show gateway service status
  pioneer issue-superuser-token  Generate a superuser JWT and print it
  pioneer stop                Stop and uninstall the persistent gateway service
  pioneer version [--json]    Show pioneer version
  pioneer help                Show this help"
}
