...
use pioneer_client::skills::types::{SkillId, SkillPackId};
...
impl SkillsCatalogView {
    ...
    pub(super) fn confirm_uninstall_skill_pack(
        ...
        self.native_task = Some(cx.spawn(move |this: WeakEntity<Self>, cx: &mut AsyncApp| {
            let mut cx = cx.clone();
            async move {
        ...
        self.native_task = Some(cx.spawn(move |this: WeakEntity<Self>, cx: &mut AsyncApp| {
            ...
            async move {
                ...
                let paths = match selection {
                    ...
                    Err(error) => {
                        let _ = this.update(&mut cx, |view, cx| {
                            view.presentation_error = Some(format!(
                                "{}: {error:#}",
