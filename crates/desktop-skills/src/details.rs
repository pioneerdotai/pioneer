...
impl SkillsCatalogView {
    pub(crate) fn render_skill_details(
        ...
        let Some(skill_id) = self.input.navigation_input.skill_id().cloned() else {
            return v_flex()
                .size_full()
        ...
        let Some(skill) =
            skill_catalog::find_skill(self.input.installed_skills.as_slice(), &skill_id).cloned()
        else {
            return v_flex()
                .size_full()
                .bg(cx.theme().background)
        ...
        let health_detail =
            skill_health::skill_health_detail(&self.input.skills_health_details, &skill.skill_id)
                .cloned();
        ...
        v_flex()
                    ...
                    .when(can_manage, |this| {
                        this.child(
                            h_flex()
                                ...
                                .gap_2()
                                .child(
                                    Button::new(self.ui_id(&skill.skill_id.to_string(), "enabled"))
                                        .xsmall()
                                        .compact()
                                        .h_6()
                                        .px_3()
                                ...
                                .child(
                                    Button::new(
                                        self.ui_id(&skill.skill_id.to_string(), "implicit"),
                                    )
                                    .xsmall()
                                    .compact()
                                    .h_6()
            ...
            .child(
                v_flex()
                    .id(self.ui_id(&skill.skill_id.to_string(), "details-scroll"))
                    .flex_1()
                    .overflow_y_scroll()
                    .p_6()
                    .gap_6()
    ...
    fn skill_details_meta_grid_columns(&self, window: &Window, _cx: &App) -> u16 {
    ...
    fn skill_details_diagnostics_grid_columns(&self, window: &Window, _cx: &App) -> u16 {
        let viewport_width = window.viewport_size().width;
        let sidebar_width = self.sidebar_width;
    ...
    fn toggle_skills_details_section(&mut self, section_id: &str, cx: &mut Context<Self>) {
        let sections = self
            .skills_details_expanded_sections
            .entry(
                self.input
                    .navigation_input
                    .skill_id()
    ...
    fn render_collapsible_skills_details_section(
        ...
        let parent = self
            ...
            .map(ToString::to_string)
            .unwrap_or_default();
        let toggle_id = self.ui_id(&parent, &format!("section:{section_id}"));
    ...
    fn render_dependency_section(
        ...
        let content = if diagnostics.is_empty() {
            ...
            let items = skill_presentation::skill_dependency_cards(diagnostics);
            let keys = crate::identity::occurrences(
                items
                    .iter()
                    .map(|item| format!("{:?}:{:?}", item.kind, item.requirement_name)),
            );
            let parent = self
                .input
                .navigation_input
            ...
            let cards = items
                ...
                .zip(keys)
                .map(|(item, key)| {
                    Self::render_dependency_card(
                        item,
                        self.ui_id(&parent, &format!("dependency:{key}"))
                            .to_string(),
                        cx,
                    )
                })
    ...
    fn render_security_section(
        ...
        let content = if findings.is_empty() {
            ...
            let items = skill_presentation::skill_security_cards(findings);
            let keys = crate::identity::occurrences(
                items
                    .iter()
                    .map(|item| format!("{:?}:{:?}", item.rule_id, item.location)),
            );
            let parent = self
                .input
                .navigation_input
            ...
            let cards = items
                ...
                .zip(keys)
                .map(|(item, key)| {
                    Self::render_security_card(
                        item,
                        self.ui_id(&parent, &format!("security:{key}")).to_string(),
                        cx,
                    )
                })
                .collect::<Vec<_>>();
    ...
    fn render_trust_gate_section(
        ...
        let content = if trust_gate.is_empty() {
            ...
            let items = skill_presentation::skill_trust_gate_cards(trust_gate);
            let keys = crate::identity::occurrences(
                items.iter().map(|item| format!("{:?}", item.tool_kind)),
            );
            let parent = self
                .input
                .navigation_input
            ...
            let cards = items
                ...
                .zip(keys)
                .map(|(item, key)| {
                    Self::render_trust_gate_card(
                        item,
                        self.ui_id(&parent, &format!("trust_gate:{key}"))
                            .to_string(),
                        cx,
                    )
                })
    ...
    fn render_diagnostics_card_field(
        ...
        let hint_id = Self::diagnostics_element_hash(&[scope, field_key, "hint"]);
        let value_id = Self::diagnostics_element_hash(&[scope, field_key, "value"]);
    ...
    fn render_diagnostics_card_secondary_field(
        ...
        let hint = title_hint.trim().to_owned();
        let hint_id = Self::diagnostics_element_hash(&[scope, field_key, "hint"]);
        let value_id = Self::diagnostics_element_hash(&[scope, field_key, "value-secondary"]);
    ...
    fn render_diagnostics_badge(
        ...
        let badge_id = Self::diagnostics_element_hash(&[scope, field_key, "badge"]);
    ...
    pub(crate) fn sync_skill_diagnostics_tables(
        &self,
    ...
    fn build_audit_table_model(audit: &[SkillAuditTimelineItem]) -> SkillDiagnosticsTableModel {
        ...
        let keys = crate::identity::occurrences(
            audit
                .iter()
                .take(8)
                .map(|item| format!("{}:{}", item.created_at, item.action)),
        );
        SkillDiagnosticsTableModel {
            columns,
            rows,
            keys,
        }
