use super::format_artifact_size;
use super::{ThreadArtifactActionStatus, ThreadArtifactFilter, ThreadArtifactsView};
...
impl ThreadArtifactsView {
    pub(crate) fn render_thread_artifacts_panel(&self, cx: &mut Context<Self>) -> AnyElement {
        ...
        let total_count = self.active_thread_artifact_items().len();
        let visible_items = self.visible_thread_artifacts();
        let visible_count = visible_items.len();
        let selected_artifact_id = self.selected_artifact_id.as_deref().map(str::to_owned);
        let is_loading_active_thread = self.thread_artifact_loading();
        ...
        v_flex()
            ...
            .child(
                v_flex()
                    ...
                    .when_some(self.thread_artifact_error(), |this, error| {
                        this.child(
                            v_flex()
                                .gap_1()
                                .rounded_md()
    ...
    fn render_thread_artifact_row(
        ...
        let selected = self.selected_artifact_id.as_deref() == Some(artifact.artifact_id.as_str());
        let preview_image_path = self.thread_artifact_preview_path(artifact, false);
    ...
    fn render_thread_artifact_detail(
        ...
        let action_status = self.thread_artifact_action_status(artifact);
        let action_in_progress = action_status.as_ref().is_some_and(|s| s.is_in_progress());
        ...
        let preview_image_path = self.thread_artifact_preview_path(artifact, true);
...
fn detail_row(label: String, value: String, _cx: &mut Context<ThreadArtifactsView>) -> AnyElement {
    h_flex()
        .w_full()
        .items_start()
        .gap_2()
...
fn artifact_action_button(
    button: Button,
    enabled: bool,
    cx: &mut Context<ThreadArtifactsView>,
) -> AnyElement {
    ...
    h_flex()
        ...
        .child(if failed {
            ...
            crate::qualification_diagnostics::spinner!(
                pioneer_client::timeline::diagnostics::AnimationSourceId::ThreadArtifactAction,
            )
