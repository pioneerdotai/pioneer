...
use gpui_kit::*;
use pioneer_client::composer::permissions::TurnPermissionMode;
...
impl ComposerView {
    ...
    pub(super) fn render_composer_permission_selector(&self, cx: &mut Context<Self>) -> AnyElement {
        ...
        let selected_mode = if options
            .iter()
            .any(|option| option.mode == self.composer_domain().selected_permission_mode)
        {
            self.composer_domain().selected_permission_mode
        } else {
            options
                .last()
