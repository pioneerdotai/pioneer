...
use crate::composer::ComposerAttachmentUploadState;
use crate::composer::ComposerCapability;
use crate::composer::ComposerCapabilityKind;
use crate::composer::ComposerView;
use crate::composer::voice::DesktopVoiceEntryAvailability;
use crate::member_picker::MemberPicker;
...
use gpui_kit::*;
use pioneer_client::composer::skill_selection::ComposerSkillChip;
use pioneer_client::composer::skill_selection::ComposerSkillChipKind;
...
impl ComposerView {
    pub(crate) fn render_composer(&mut self, cx: &mut Context<Self>) -> AnyElement {
        ...
        let attachments = self.composer_domain().attachments.clone();
        ...
        let microphone_error = self.desktop_microphone_error_message(cx);
        let editing_message = self.composer_edit_target().is_some();
        let message_mode = self.composer_domain().selected_mode == ThreadMode::Message;
        let active_thread_snapshot = self.active_thread_snapshot();
        let gateway_connected =
            self.connection_state == crate::composer::GatewayConnectionState::Connected;
        ...
        let can_send = if let Some(target) = self.composer_edit_target() {
            gateway_connected
        ...
        let has_cli_runtime_steer_target = has_in_flight_turn
            && self
                .client
                .thread_snapshot(&self.thread_id)
                .is_some_and(|snapshot| snapshot.cli_binding().is_some())
            && self
                .composer_input
                .as_ref()
                .and_then(|p| p.runtime_selection())
        ...
        let desktop_voice_hold_ui_active = self.voice.read(cx).desktop_voice_hold_ui_active();
        let desktop_voice_send_processing = self.voice.read(cx).desktop_voice_send_processing();
        let voice_entry_availability = if editing_message || (has_in_flight_turn && !message_mode) {
            DesktopVoiceEntryAvailability::Hidden
        } else {
            self.voice.read(cx).desktop_voice_entry_availability()
        };
                    ...
                    .child(
                        v_flex()
                            ...
                            .on_mouse_move(cx.listener(|view, event: &MouseMoveEvent, _, cx| {
                                view.voice.update(cx, |voice, cx| voice.update_desktop_voice_hold_pointer(event.position, cx));
                            }))
                            .on_mouse_up(
                                MouseButton::Left,
                                cx.listener(|view, event: &MouseUpEvent, _, cx| {
                                    view.voice.update(cx, |voice, cx| voice.release_desktop_voice_hold_at(event.position, cx));
                                }),
                            )
                            .on_mouse_up_out(
                                MouseButton::Left,
                            ...
                            .child(
                                v_flex()
                                    ...
                                    .child(if desktop_voice_hold_ui_active {
                                        self.voice.update(cx, |voice, cx| voice.render_desktop_voice_hold_prompt(cx))
                                    } else {
                                                ...
                                                this.child(
                                                    Button::new("stop-turn-while-messaging")
                                                        ...
                                                        .loading(crate::qualification_diagnostics::observed_loading!(
                                                            pioneer_client::timeline::diagnostics::AnimationSourceId::ComposerCancelTurnButton,
                                                            is_cancelling,
                                                        ))
                                            ...
                                            .child(match composer_primary_action {
                                                ...
                                                | DesktopComposerPrimaryAction::Stop => {
                                                    Button::new(composer_action_id)
                                                        ...
                                                        .loading(crate::qualification_diagnostics::observed_loading!(
                                                            pioneer_client::timeline::diagnostics::AnimationSourceId::ComposerPrimaryActionButton,
    ...
    pub(crate) fn insert_composer_mention(
        ...
        self.composer_domain_intent(ComposerDomainAction::SelectMention { candidate });
        if let Some(input) = &self.composer_input {
            let text = input.draft().text.clone();
    ...
    fn render_composer_chip_badges(&self, cx: &mut Context<Self>) -> AnyElement {
        ...
        for attachment in self.composer_domain().attachments.iter().cloned() {
            chips.push(self.render_composer_attachment_badge(attachment, cx));
        }

        let effective_capabilities = self.effective_composer_capabilities();
        for chip in self.composer_skill_chips() {
            chips.push(self.render_composer_skill_selection_badge(chip, cx));
        }
        for capability in effective_capabilities {
            chips.push(self.render_composer_capability_badge(capability, cx));
        }
    ...
    fn render_composer_attachment_badge(
        &self,
        attachment: crate::composer::ComposerAttachment,
        cx: &mut Context<Self>,
    ) -> AnyElement {
        let identity = attachment.path.clone();
        let group_id = format!("composer-attachment-chip-{identity}");
        ...
        h_flex()
            .id(SharedString::from(group_id.clone()))
            .flex_initial()
            .max_w(px(196.))
            .min_w_0()
            .h(px(32.))
                ...
                div()
                    ...
                    .child(if is_uploading {
                        crate::qualification_diagnostics::spinner!(
                            pioneer_client::timeline::diagnostics::AnimationSourceId::ComposerAttachmentUpload,
                        )
            ...
            .child(
                Button::new(SharedString::from(format!(
                    "composer-attachment-remove-{identity}"
                )))
                .flex_none()
                .ghost()
                .xsmall()
                .compact()
                .icon(IconName::Close)
                .disabled(self.composer_upload_in_progress() || self.desktop_voice_context_locked())
                .rounded_full()
                .on_click(cx.listener(move |view, _, _, cx| {
                    if view.remove_composer_attachment(identity.clone()) {
                        cx.notify();
                    }
                })),
            )
    ...
    fn render_composer_capability_badge(
        ...
        h_flex()
            ...
            .child(
                Button::new(SharedString::from(format!(
                    "composer-capability-remove-{identity}"
                )))
                .flex_none()
                .ghost()
                .xsmall()
                ...
                .rounded_full()
                .on_click(cx.listener(move |view, _, _, cx| {
                    if view.remove_composer_capability(identity.clone()) {
                        cx.notify();
                    }
                })),
            )
    ...
    fn composer_skill_chips(&self) -> Vec<ComposerSkillChip> {
        ...
        project_composer_skill_chips(self.composer_domain().skill_selections.as_slice(), &picker)
    }
    ...
    fn render_composer_skill_selection_badge(
        ...
        let identity = chip.key.clone();
        let group_id = format!("composer-skill-selection-chip-{identity}");
        ...
        h_flex()
            ...
            .child(
                Button::new(SharedString::from(format!(
                    "composer-skill-selection-remove-{identity}"
                )))
                .flex_none()
                .ghost()
                .xsmall()
                ...
                .rounded_full()
                .on_click(cx.listener(move |view, _, _, cx| {
                    if view.remove_composer_skill_selection(selection.clone()) {
                        cx.notify();
                    }
                })),
            )
