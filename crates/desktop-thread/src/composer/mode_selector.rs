...
use gpui_kit::*;
use pioneer_client::composer::message_edit::ComposerMessageEditTarget;
...
use pioneer_client::composer::state_machine::ComposerReplyTarget;
...
impl ComposerView {
    ...
    pub(crate) fn render_composer_mode_selector(&self, cx: &mut Context<Self>) -> AnyElement {
        let selected_mode = self.composer_domain().selected_mode;
        let desktop_entity = cx.entity();
        let content = if let Some(target) = self.composer_edit_target().cloned() {
            vec![self.render_composer_edit_target(target, cx)]
        } else if let Some(target) = self.composer_domain().reply_target.clone() {
                    ...
                    let button = Button::new(id)
                        ...
                        .on_click(move |_, _, cx| {
                            let _ = desktop_entity.update(cx, |view, cx| {
                                let transition = view.composer_domain_intent(
            ...
            .child(
                Button::new("cancel-composer-reply")
                    ...
                    .on_click(cx.listener(|view, _, _, cx| {
                        view.composer_domain_intent(ComposerDomainAction::ClearReplyTarget);
                        cx.notify();
                    })),
            )
            .into_any_element()
    ...
    fn render_composer_edit_target(
        &self,
        target: ComposerMessageEditTarget,
        cx: &mut Context<Self>,
    ) -> AnyElement {
...
mod tests {
    ...
    fn desktop_reply_replaces_mode_buttons_with_truncated_cancelable_context() {
        let source = include_str!("mode_selector.rs");
        assert!(source.contains("self.composer_domain().reply_target.clone()"));
