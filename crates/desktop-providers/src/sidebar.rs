const PROVIDERS_FILTER_API_NODE_ID: &str = "providers:api";
const PROVIDERS_FILTER_CLI_NODE_ID: &str = "providers:cli";
const PROVIDERS_FILTER_CONNECTED_NODE_ID: &str = "providers:connected";
use crate::providers::{ProviderCatalogView, ProviderFilter};
                ...
                match parse_providers_sidebar_node_key(item_id) {
                    ProvidersSidebarNodeKey::Filter(filter) => {
                        ...
                        ListItem::new(SharedString::from(format!(
                            "providers:{mount}:{scope}:sidebar:{item_id}:row"
                        )))
                        .on_click({
                            let owner = desktop_entity.clone();
                            move |_, _, cx| {
                                let _ = owner
                                    .update(cx, |view, cx| view.set_provider_filter(filter, cx));
                            }
                        })
                    ...
                    ProvidersSidebarNodeKey::Unknown => ListItem::new(SharedString::from(format!(
                        "providers:{mount}:{scope}:sidebar:{item_id}:row"
                    )))
                    .separator()
                    .h(px(TREE_ROW_HEIGHT_PX))
...
pub(crate) struct ProviderSidebar {
    owner: WeakEntity<ProviderCatalogView>,
    provider_tree_state: Entity<gpui_kit::component::tree::TreeState>,
    mount: u64,
...
impl ProviderSidebar {
    pub(crate) fn new(
        ...
        cx.new(|cx| Self {
            ...
            mount,
            selection: None,
            provider_tree_state: cx.new(|cx| gpui_kit::component::tree::TreeState::new(cx)),
        })
    }
    pub(crate) fn sync(
        &mut self,
        ...
        if self
            .selection
            .as_ref()
            .is_some_and(|p| p.scope == scope && p.filter == filter && p.can_manage == can_manage)
        {
            return;
        }
        self.selection = Some(SidebarSelection {
            scope,
            filter,
            can_manage,
        });
        ...
        self.provider_tree_state.update(cx, |tree, cx| {
            tree.set_items(items, cx);
            tree.set_selected_index(Some(selectors::provider_filter_tree_index(filter)), cx);
        });
        cx.notify();
    }
}
impl Render for ProviderSidebar {
