...
impl CatalogSidebar {
                    ...
                    .child(
                        Button::new("mcp-sidebar-refresh")
                            ...
                            .on_click(cx.listener(|view, _, _, cx| {
                                let _ = view
                                    .owner
                                    .update(cx, |owner, cx| owner.refresh_mcp_servers(cx));
                                cx.notify();
                            })),
                    ),
            )
    ...
    pub(crate) fn render_mcp_details_sidebar(&self, cx: &mut Context<Self>) -> AnyElement {
        ...
        let is_pending = selected
            .as_ref()
            .is_some_and(|server| self.is_mcp_pending(server.id.as_str()));
                            ...
                            Button::new("mcp-details-sidebar-restart")
                                ...
                                .on_click({
                                    ...
                                    move |_, _, cx| {
                                        ...
                                        let _ = desktop_entity.update(cx, |view, cx| {
                                            view.restart_mcp_server(selected.id.clone(), cx);
                                            cx.notify();
                                        });
                                    }
                                }),
...
fn sidebar_icon(icon: IconName, cx: &mut Context<CatalogSidebar>) -> AnyElement {
    let icon_bg = cx.theme().foreground.opacity(0.075);
    div()
        .size_6()
        .rounded_full()
...
pub(crate) struct CatalogSidebar {
    pub owner: WeakEntity<McpCatalogView>,
    pub input: Arc<CatalogInput>,
}
impl Deref for CatalogSidebar {
    type Target = CatalogInput;
    fn deref(&self) -> &CatalogInput {
        &self.input
    }
}
impl Render for CatalogSidebar {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        if matches!(
            self.navigation_input.destination(),
            pioneer_client::navigation::SemanticDestination::Mcp { server_id: Some(_) }
        ) {
