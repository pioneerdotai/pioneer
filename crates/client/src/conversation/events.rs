...
pub fn turn_item_id(item: &TurnItem) -> &str {
    match item {
...
pub fn turn_item_type(item: &TurnItem) -> &'static str {
    match item {
...
mod tests {
    ...
    #[test]
    fn conversation_event_helpers_extract_ids_kind_and_payload() {
        ...
        assert_eq!(event.thread_id(), Some("thread_1"));
        assert_eq!(event.turn_id(), Some("turn_1"));
        assert_eq!(event.item_id(), None);
        assert_eq!(event.kind(), EventKind::LocalTurnStartRequested);
        assert_eq!(
            event.payload_value()["LocalTurnStartRequested"]["turn_id"],
            "turn_1"
        );

        let envelope = EventEnvelope {
            sequence: 1,
            thread_id: event.thread_id().unwrap_or_default().to_owned(),
        ...
        assert_eq!(envelope.thread_id, "thread_1");
        assert_eq!(envelope.turn_id.as_deref(), Some("turn_1"));
    }
}
