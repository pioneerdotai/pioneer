...
impl ThreadSidebarView {
    pub(crate) fn open_rename_workspace_dialog(
        &mut self,
        workspace_id: String,
        window: &mut Window,
        cx: &mut Context<Self>,
        ...
        let rename_workspace: Rc<dyn Fn(&mut App) -> bool> = Rc::new({
            ...
            move |cx| {
                ...
                *field_error.borrow_mut() = None;
                ...
                if let Some(error) =
                    error.unwrap_or_else(|_| Some(t!("workspace.error.create_failed").to_string()))
                {
        ...
        (self.rename_workspace_dialog)(
            Rc::new(move |dialog, window, cx| {
